...
! SPRNG
    integer seed,gtype,junk,streamnum,nstreams
...
!!!!!!x`
  ...
  do it = 0, nit
      ...
      call updatevaclist (nrow,ncol, mesh,totnvac, vac )
            call yLatex(nrow,ncol,mesh, vac,totnvac ,stream) ! Make one lattice exchange
!     write(*,*)nvac 
...
!--------------------
     ...
     call MPI_send(mesh(nrow,0),1,MPI_TYPE_ROW,down,0,MPI_COMM_2D,ierror)
        ...
        if(any(black==rank))then 
     call updatevaclist (nrow,ncol, mesh,totnvac, vac )
            call xLatex(nrow,ncol,mesh, vac, totnvac,stream ) ! Make one lattice exchange
